...
if interactionActive
{
	...
	if response != noone
	{
		dialogScrollIndex = 0;
		dialogCrawlIndex = 0;
		leaveCall = script_execute(DialogCache.scripts[| dialogIndex]);
...
while run 
{
	if !ds_stack_empty(mainCutsceneStack)
	{
		...
		if cutsceneTimer == 0
		{
			var debugString = submap[? cutsceneCommandData.debugString];
			var debugBool = GameManager.cutsceneDebug;
			if debugString != noone && debugBool
			{
				message_feed_add(debugString,noone);
			}
		}
		...
		switch type
		{
			...
			case cutsceneCommand.fadeOut:				#region
				...
				finished = true;
				break;									#endregion
			case cutsceneCommand.holdFaded:				#region
				var colour = submap[? cutsceneCommandData.arg1];
				var duration = submap[? cutsceneCommandData.arg2];
				Camera.fadeTypeState = fadeType.hold;
				Camera.fadeColour = colour;
				Camera.fadeDuration = duration;
				Camera.fadeTimer = 0;
				finished = true;
			...
			case cutsceneCommand.commandWorldEvent:		#region
				...
				finished = true;
				break;									#endregion
			case cutsceneCommand.dialogInitiate:		#region
				var index = submap[? cutsceneCommandData.arg1];
				var indexSub = submap[? cutsceneCommandData.arg2];
				var type = submap[? cutsceneCommandData.arg3];
				scr_dialog_initiate(index,indexSub,type);
				finished = true;
				break;									#endregion
			case cutsceneCommand.dialogWait:			#region
				if dialogCurrentState == dialogState.closed finished = true;
				break;									#endregion
		}
		cutsceneTimer++;
			...
			if stack != undefined && ds_exists(stack,ds_type_stack)
			{
				...
				switch commandType
				{
					...
					case puppetCommand.moveTo:			#region Move to
							//action
						if abs(commandArg2-x) > abs(xSpd)*1.2
						{
