...
PlayerStats.GCS[? class] = class_get_stat(class,weaponClassStats.groundComboLength);
PlayerStats.GFS[? class] = class_get_stat(class,weaponClassStats.groundFinisherLength);
PlayerStats.ACS[? class] = class_get_stat(class,weaponClassStats.aerialComboLength);
PlayerStats.AFS[? class] = class_get_stat(class,weaponClassStats.aerialFinisherLength);

	//alter caches data (defaults and removes) for the normal 4
		//ground combos
			//cut down to needed size
while ds_list_size(GCCache) > PlayerStats.GCS[? class] ds_list_delete(GCCache,ds_list_size(GCCache))
...
for (var i = 0; i < PlayerStats.GCS[? class]; i++)
{
	if GCCache[| i] == undefined GCCache[| i] = class_get_stat(class,weaponClassStats.groundComboDefault);
		...
		//ground finishers
			//cut down to needed size
while ds_list_size(GFCache) > PlayerStats.GFS[? class] ds_list_delete(GFCache,ds_list_size(GFCache))
			//check if all defined, if not, set to default as defined in ComboCache
for (var i = 0; i < PlayerStats.GFS[? class]; i++)
{
	if GFCache[| i] == undefined GFCache[| i] = class_get_stat(class,weaponClassStats.groundFinisherDefault);
		...
		//aerial combos
			//cut down to needed size
while ds_list_size(ACCache) > PlayerStats.ACS[? class] ds_list_delete(ACCache,ds_list_size(ACCache))
			//check if all defined, if not, set to default as defined in ComboCache
for (var i = 0; i < PlayerStats.ACS[? class]; i++)
{
	if ACCache[| i] == undefined ACCache[| i] = class_get_stat(class,weaponClassStats.aerialComboDefault);
		...
		//aerial finishers
			//cut down to needed size
while ds_list_size(AFCache) > PlayerStats.AFS[? class] ds_list_delete(AFCache,ds_list_size(AFCache))
			//check if all defined, if not, set to default as defined in ComboCache
